...
app.post('/api/vehicles/:vehicleId/break-unit', async (req, res) => {
  try {
    ...
    const sourceLayerIndex = packagingStructure.findIndex(p => p.unit === unit);
    const targetLayerIndex = packagingStructure.findIndex(p => p.unit === targetUnit);
    ...
    const sourceLayer = packagingStructure[sourceLayerIndex];
    const targetLayer = packagingStructure[targetLayerIndex];

    // Conversion ratio is based on qty field in packaging structure
    let conversionRatio = 1;
    if (sourceLayerIndex < targetLayerIndex) {
      // Breaking down from larger to smaller
      for (let i = sourceLayerIndex + 1; i <= targetLayerIndex; i++) {
        conversionRatio *= (packagingStructure[i].qty || 1);
      }
    } else {
      return res.status(400).json({
        error: 'Can only break down from larger units to smaller units'
      });
    }
